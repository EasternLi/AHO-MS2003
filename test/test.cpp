...
void SmallGen1(const string &name, function<void(const tuple<Graph> &)> cs) {
	...
	for(int i = 1; i <= n; i++) {
		cs(TinyGen(5, {1,10}, 0, { -2, 2 }, { -2, 6 }, { -10, 10 }, i));
	}
}
...
void SmallGen2(const string &name, function<void(const tuple<Graph> &)> cs) {
	int n = (name.substr(0, 7) == "Compare" ? 10000 : 100);
	for(int i = 1; i <= n; i++) {
		cs(TinyGen(4, {1,10}, 1, { -2, 2 }, { -2, 6 }, { -10, 10 }, i));
	}
}

void MeduimGen1(const string &name, function<void(const tuple<Graph> &)> cs) {
	int n = (name.substr(0, 7) == "Compare" ? 100 : 1);
	for(int i = 1; i <= n; i++) {
		cs(TinyGen(20, {1,50}, 0, { -2, 2 }, { -2, 6 }, { -10, 10 }, i));
	}
}

void MeduimGen2(const string &name, function<void(const tuple<Graph> &)> cs) {
	int n = (name.substr(0, 7) == "Compare" ? 100 : 1);
	for(int i = 1; i <= n; i++) {
		cs(TinyGen(20, {1,50}, 1, { -2, 2 }, { -2, 6 }, { -10, 10 }, i));
	}
}

void MeduimGen3(const string &name, function<void(const tuple<Graph> &)> cs) {
	int n = (name.substr(0, 7) == "Compare" ? 10 : 1);
	for(int i = 1; i <= n; i++) {
		cs(TinyGen(50, {1,20}, 1, { -4, 4 }, { -4, 4 }, { -10, 10 }, i));
	}
}
...
// int Speed(const tuple<int> &in){
// 	int a=0;
// 	for(int i=1;i<=get<0>(in);i++){
// 		a++;
// 	}
// 	return a;
// }

// void SpeedCheck(const int &res, const tuple<int> &in){
...
// void SpeedGen(const string &name, function<void(const tuple<int> &)> cs){
// 	cs(150000000);
// }

// TESTA_DEF_VERIFY_WITH_TB(SpeedTest, SpeedGen, SpeedCheck, Speed);
